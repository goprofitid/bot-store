...
export default async function Message(hisoka, m, chatUpdate) {
    try {
		...
		if(/^pulsa/i.test(m.body)){
		    fs.readFile('./temp/pulsa.txt', 'utf8', (err, data) => {
			if(err){
			    m.reply(err)
			}
			m.reply(data)
		...
		if(/^rdp/i.test(m.body)){
		    fs.readFile('./temp/rdp.txt', 'utf8', (err, data) => {
			if(err){
			    m.reply(err)
			}
			m.reply(data)
        ...
        if(/^convert/i.test(m.body)){
		    fs.readFile('./temp/convert.txt', 'utf8', (err, data) => {
			if(err){
			    m.reply(err)
			}
			m.reply(data)
		...
		if(/^pay/i.test(m.body)) {
			let text = `Hello @${m.sender.split`@`[0]}, Berikut Metode Jenis Pembayaran \n\n`
			text += `❚❏ 🛍 QRIS\n`
            text += '*A.N DENDI RAMADHAN*\n'
			text += `❚❏ 🛍 Bank BCA\nNo.Rek : 1660579105\n`
			text += `❚❏ 🛍 Bank BNI\nNo.Rek : 1229258943\n`
			text += `❚❏ 🛍 Bank MANDIRI\nNo.Rek : 1330027727122\n`
			text += `❚❏ 🛍 DANA\nNo.Rek : 0895415430305\n`
			text += `❚❏ 🛍 GOPAY\nNo.Rek : 0895415430305\n`
            m.reply('https://det.my.id/upload/610bdc4bac79ec3f1bc6d1fec96b4193.jpg', { caption: text })
        }
